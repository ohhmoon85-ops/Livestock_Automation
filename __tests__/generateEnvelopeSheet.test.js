...
// ── T6: 검수서 출현 순서 보존 ──────────────────────────────────────────────
test("T6: 학교 그룹 순서 — 가나다순이 아닌 검수서 첫 출현 순서 보존", () => {
  const wb = new ExcelJS.Workbook();
  const ws = wb.addWorksheet("검수서");
  ws.addRow(["날짜"]);
  ws.addRow(["담당자", "매출처(고객)명", "품목명", "규격", "수량"]);
  ...
  // 가나다순(구룡→동백→해성)이 아닌 검수서 출현 순서: 해성 → 구룡 → 동백
  expect(envSheet.getRow(2).getCell("B").value).toBe("해성여자고등학교");
  expect(envSheet.getRow(4).getCell("B").value).toBe("구룡초등학교");
  expect(envSheet.getRow(6).getCell("B").value).toBe("동백유치원");
});
...
test("T11: 5월_4일 완성본 통합 시나리오", async () => {
  ...
  // b. 검수서 첫 출현 순서와 봉투 학교 순서 일치
  const srcRows11 = XLSX.utils.sheet_to_json(xlsxWb.Sheets["검수서"], {
    header: 1,
    defval: "",
  });
  let srcHdr11 = -1, srcCustCol11 = -1, srcQtyCol11 = -1;
  for (let i = 0; i < srcRows11.length; i++) {
    const ci = srcRows11[i].findIndex((v) => String(v).includes("매출처"));
    const qi = srcRows11[i].findIndex((v) => String(v).trim() === "수량");
    if (ci !== -1 && qi !== -1) {
      srcHdr11 = i; srcCustCol11 = ci; srcQtyCol11 = qi; break;
    }
  }
  const expectedOrder11 = [];
  for (let i = srcHdr11 + 1; i < srcRows11.length; i++) {
    const v = srcRows11[i][srcQtyCol11];
    if (typeof v !== "number" || isNaN(v)) continue;
    const key = cleanSchoolName(String(srcRows11[i][srcCustCol11] ?? ""));
    if (key && !expectedOrder11.includes(key)) expectedOrder11.push(key);
  }
  expect(groupRows).toEqual(expectedOrder11);
...
// ── T17: 소수점 수량 보존 — numFmt='0.##', 정수 변환 없음 ────────────────────
test("T17: 소수점 수량 보존 및 numFmt 설정", () => {
  const wb = new ExcelJS.Workbook();
  const ws = wb.addWorksheet("검수서");
  ws.addRow(["날짜"]);
  ws.addRow(["담당자", "매출처(고객)명", "품목명", "규격", "수량"]);
  ws.addRow(["", "학교A", "한우[양지]",            "", 12.5]);
  ws.addRow(["", "학교B", "무항생제계란[유정란]",   "", 7.2]);
  ws.addRow(["", "학교C", "돈육[목살]",             "", 6]);

  generateEnvelopeSheet(wb);
  const envSheet = wb.getWorksheet("봉투");

  // 학교A 품목 행: row 3
  const cellA = envSheet.getRow(3).getCell("C");
  expect(typeof cellA.value).toBe("number");
  expect(cellA.value).toBe(12.5);   // 13으로 반올림되지 않음
  expect(cellA.numFmt).toBe("0.##");

  // 학교B 품목 행: row 5
  const cellB = envSheet.getRow(5).getCell("C");
  expect(cellB.value).toBe(7.2);
  expect(cellB.numFmt).toBe("0.##");

  // 학교C 품목 행: row 7 — 정수도 정상 보존
  const cellC = envSheet.getRow(7).getCell("C");
  expect(cellC.value).toBe(6);
  expect(cellC.numFmt).toBe("0.##");
});

// ── T18: 5월 6일 파일 — 봉투 학교 순서 = 검수서 출현 순서 ───────────────────
test("T18: 5월 6일 파일 봉투 학교 순서가 검수서 출현 순서와 일치", async () => {
  const filePath = path.resolve(
    __dirname,
    "../../5월 6일(수) 출고리스트_봉투 테스트.xlsx"
  );
  ...
  let srcHdr = -1, srcCustCol = -1, srcQtyCol = -1;
  for (let i = 0; i < srcRows.length; i++) {
    const ci = srcRows[i].findIndex((v) => String(v).includes("매출처"));
    const qi = srcRows[i].findIndex((v) => String(v).trim() === "수량");
    if (ci !== -1 && qi !== -1) { srcHdr = i; srcCustCol = ci; srcQtyCol = qi; break; }
  }
  const expectedOrder = [];
  for (let i = srcHdr + 1; i < srcRows.length; i++) {
    const v = srcRows[i][srcQtyCol];
    if (typeof v !== "number" || isNaN(v)) continue;
    const key = cleanSchoolName(String(srcRows[i][srcCustCol] ?? ""));
    if (key && !expectedOrder.includes(key)) expectedOrder.push(key);
  }

  expect(envSchools).toEqual(expectedOrder);
});
