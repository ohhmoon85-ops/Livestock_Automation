...
export function generateEnvelopeSheet(workbook) {
  ...
  srcSheet.eachRow((row, rowNum) => {
    ...
    if (!groups.has(schoolKey)) groups.set(schoolKey, { rows: [] });
    groups.get(schoolKey).rows.push({ itemName: itemRaw, mealCode, qty: rawQty });
  });

  const sortedKeys = [...groups.keys()].sort((a, b) => a.localeCompare(b, 'ko'));
  ...
  for (const schoolKey of sortedKeys) {
    ...
    const totalLines = 1 + group.rows.length;
    ...
    for (const rowData of group.rows) {
      const displayName = convertBrackets(rowData.itemName);
      const mealStr     = rowData.mealCode;
