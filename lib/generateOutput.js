...
export async function generateOutput(originalRows, matchResults, headerRowIndex = 0) {
  ...
  for (let i = headerRowIndex + 1; i < originalRows.length; i++) {
    ...
    if (res && res._matchedEntries && res._matchedEntries.length > 0) {
      ...
      const joinDed = (key) => [...new Set(entries.map((e) => e[key] ?? "").filter(Boolean))].join("\n");

      // 발급일자·도축장: 중복 값 제거 후 표시
      if (COL.발급일자) rowData[COL.발급일자 - 1] = joinDed("발급일자");
      if (COL.발급번호) rowData[COL.발급번호 - 1] = join("발급번호");
      if (COL.묶음번호) rowData[COL.묶음번호 - 1] = join("묶음번호");
      if (COL.도축장)   rowData[COL.도축장   - 1] = joinDed("도축장");
      if (COL.매입처)   rowData[COL.매입처   - 1] = join("producerName");
    }
...
function styleHeader(row) {
  row.height = 22;
  // A~L(1~12)열만 서식 적용 — 빈 열까지 네이비 배경이 이어지지 않도록
  for (let c = 1; c <= 12; c++) {
