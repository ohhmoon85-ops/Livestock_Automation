...
import { generateEnvelopeSheet } from "./generateEnvelopeSheet.js";
 ...
 */
export async function generateOutput(originalRows, matchResults, headerRowIndex = 0, inputWb) {
  ...
  // ── [검수서] 복사 (값만, 시트 순서: 검수서 → 매출 시트 → 봉투) ──
  if (inputWb && inputWb.Sheets["검수서"]) {
    const inspectionSheet = workbook.addWorksheet("검수서");
    const inspRows = XLSX.utils.sheet_to_json(
      inputWb.Sheets["검수서"],
      { header: 1, defval: null }
    );
    for (const row of inspRows) {
      inspectionSheet.addRow(row);
    }
  }
  ...
  generateEnvelopeSheet(workbook);
