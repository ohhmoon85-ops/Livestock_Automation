...
/**
 * @param {Array} originalRows  - 원본 2D 배열 (rows[0] = 헤더)
 * @param {Array} matchResults  - matchAll() 결과의 results
 * @returns {Buffer} xlsx Buffer
 */
export async function generateOutput(originalRows, matchResults) {
  ...
  const headers = originalRows[0].map((h) => (h == null ? "" : String(h)));
  ...
  // 경고 메시지 컬럼 (헤더 마지막 다음)
  const warnCol = headers.filter(Boolean).length + 1;
  ...
  for (let i = 1; i < originalRows.length; i++) {
    ...
    if (res && res._warning) {
      ...
      const warnCell = excelRow.getCell(warnCol);
      warnCell.value = res._warning;
      warnCell.font = { color: { argb: "FF856404" }, italic: true, size: 10 };
    }
